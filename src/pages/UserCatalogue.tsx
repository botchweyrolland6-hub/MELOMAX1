...
export const UserCatalogue: React.FC<UserCatalogueProps> = ({
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-16">
      <div className="relative rounded-3xl overflow-hidden glass-panel border border-slate-800 p-8 md:p-12 bg-gradient-to-r from-slate-950 via-slate-900 to-amber-950/40">
        <div className="max-w-2xl space-y-4 relative z-10">
          <span className="px-3 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs font-bold uppercase tracking-widest inline-block">
          ...
          <div className="pt-2">
            <div className="relative max-w-xl">
              <Search className="w-5 h-5 absolute left-4 top-3.5 text-amber-400" />
              <input
                type="text"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search by size (e.g. 205/55 R16, 385/65 x22.5), brand, model..."
                className="w-full pl-12 pr-4 py-3.5 bg-slate-950/90 border border-amber-500/30 rounded-2xl text-white font-medium text-sm focus:outline-none focus:border-amber-500 shadow-xl"
              />
            </div>
          </div>
        </div>

        <Disc className="absolute -right-12 -bottom-12 w-80 h-80 text-amber-500/5 rotate-45 pointer-events-none" />
      </div>
